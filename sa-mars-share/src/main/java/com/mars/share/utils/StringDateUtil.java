...
public class StringDateUtil {
    ...
    /**
     * 按指定格式返回时间,实际是舍弃某些格式 （入参是Date对象，出参也是Date对象，比如 date是new Date，则dateFormat(date,1)返回的是时分秒为0的date）
     * @param date
     * @param formatNum
     * 1 yyyyMMdd
     * 2 yyyy/MM/dd
     ...
     * @return 操作后的日期
     */
    public static Date addHours(Date startDate, int addnos) {
        if (startDate == null) {
            startDate = new Date();
        }
        Calendar cc = Calendar.getInstance();
